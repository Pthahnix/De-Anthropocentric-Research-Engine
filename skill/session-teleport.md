...
- `.mcp.json` with `dare-session` entry containing: ANTHROPIC_BASE_URL, ANTHROPIC_AUTH_TOKEN, ANTHROPIC_MODEL, GITHUB_PAT, HF_TOKEN
...
**Credential source**: Read `.mcp.json` and extract ANTHROPIC_BASE_URL, ANTHROPIC_AUTH_TOKEN, ANTHROPIC_MODEL, GITHUB_PAT, HF_TOKEN from the `dare-session` MCP server entry's `env` field.
...
Read credentials from `.mcp.json` `dare-session` entry (not `.env`).
...
Read PAT from `.mcp.json` `dare-session.env.GITHUB_PAT`.
